...
shannon_div<-ggplot(calculated_diversity_df, aes(x =Treatment, y = shannon, fill = Treatment ))+
  geom_boxplot()+
  theme_bw()+
  labs(title = "Fisher diversity indexes")
...
#export as pdf
ggsave(shannon_div, file = "./Results/3_shannon_diversity.pdf", 
       width = 15, 
       height = 15, 
       units = "cm"  )
...
# anova test
anova_shannon<-aov(shannon ~ Treatment, data = calculated_diversity_df)

# get p values
summary(anova_shannon)

# post-hoc, both Tukey and Least Significat Difference
LSD.test(y = anova_shannon, "Treatment", group=TRUE, console=TRUE, p.adj= "fdr")
